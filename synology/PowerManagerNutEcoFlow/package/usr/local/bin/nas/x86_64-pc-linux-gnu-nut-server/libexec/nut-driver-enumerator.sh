...
prefix="/home/efsz/proj/ps-003/ef-nut/x86_64-pc-linux-gnu-nut-server"
[ -n "${NUT_CONFPATH-}" ] || NUT_CONFPATH="/home/efsz/proj/ps-003/ef-nut/x86_64-pc-linux-gnu-nut-server/etc"
    ...
    if [ "${#NEW_CHECKSUM}" != 0 ]; then
        for UPSS in $NEW_CHECKSUM ; do
            ...
            if [ -n "$CURR_DRV" ] ; then
                ...
                "/home/efsz/proj/ps-003/ef-nut/x86_64-pc-linux-gnu-nut-server/bin/$CURR_DRV" -a "$UPSS" -c reload-or-error >/dev/null 2>/dev/null \
                && DO_UNREGISTER=no
            fi
